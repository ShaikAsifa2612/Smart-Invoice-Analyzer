...
def is_username_taken1(username):
    if os.path.exists(USER_DB):
        with open(USER_DB, "rb") as file:
            existing_data = pickle.load(file)
        return any(user["username"] == username for user in existing_data)
                    ...
                    st.markdown(
                        ...
                        <div class="popup-overlay">
                            <div class="popup-box">
                                ...
                                <p>Welcome to Smart Invoice Analyzer </p>
                            </div>
                        </div>
                        """,
                        unsafe_allow_html=True
    ...
    with st.container():
        ...
        if submit:
            if not category_df.empty:
                ...
                # Step 5: Display Product Details
                selected_product1 = product_counts.get(f"{selected_product}", 0)
                percentage = ((selected_product1 / max_count) * 100)
                progress = percentage
                ...
                st.subheader(f"Demand of {selected_product}: {int(percentage)}%")
                st.progress(progress / 100)
    ...
    if uploaded_file:
        ...
        if text:
            ...
            with col1:
                ...
                image = img.resize((500,400))
                st.image(image)
            with col2:
                submit = st.button("Validate Invoice")
            if submit:
                if error == 0:
                    ...
                    st.error(f"Please upload a Valid Invoice")
        ...
        with st.container():
            ...
            with col2:
                ...
                with st.form("Card2"):
                    ...
                    image = Image.open('product_insight.jpg')
                    resized_image = image.resize((700,350))
                    st.image(resized_image)
        ...
        if not load.get("uploaded_files"):
            ...
            with col2:
                for idx, image_info in enumerate(image_data):
                    st.image(image_info["image"], caption=f"Image {idx + 1}", use_column_width=True)
